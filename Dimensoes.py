...
def executaCarga():
    ...
    def load_Cargas(connect_ora, mysql_engine, origem):
        ...
        sql = "SELECT id, tabela,tipo,modulo,sequencia,string_del,string_ins FROM dw_bi.queries  WHERE db_origem = '"+origem+"'  and id = 11 AND tipo = 'D' order by tipo asc,sequencia asc;"
        data=pd.read_sql(sql,mysql_engine)
        print("Total records form Queries : ", data.shape[0])
        ...
        for i in range(len(data)):
            ...
            #executa o update na tabela para informar a data de atualização da mesma
